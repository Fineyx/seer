...
};let data=FALLBACK_DATA, currentSpecies, effectData={suit:[],title:[],mintmark:[]};
...
function effectOptions(id, items, label) { const el=$(id); el.innerHTML=`<option value="">${label}</option>`+items.map(x=>`<option value="${x.id}">${x.name}${x.desc?` · ${x.desc}`:""}</option>`).join(""); }
function loadEffects(){ fetch("data/effects.data",{cache:"no-store"}).then(r=>r.json()).then(d=>{effectData=d;effectOptions("suitSelect",d.suit,"不使用套装");effectOptions("officialTitleSelect",d.title.filter(x=>x.is_ability_bonus||x.attr_bonus),"不使用属性称号");["mark1","mark2","mark3"].forEach(id=>effectOptions(id,d.mintmark.filter(x=>x.max_attr_value),"不使用刻印"));["suitSelect","officialTitleSelect","mark1","mark2","mark3"].forEach(id=>$(id).addEventListener("change",calculate));}).catch(()=>{}); }
function addAttrs(result, attrs){if(!attrs)return;const map={hp:"hp",atk:"attack",def:"defense",sp_atk:"spAttack",sp_def:"spDefense",spd:"speed"};Object.entries(map).forEach(([from,to])=>result[to]+=Number(attrs[from]||0));}function selected(){return data.species.find(s=>s.id===$("speciesSelect").value)||data.species[0]}
...
function calculate(){
  ...
  const title=EFFECTS.titles.find(e=>e.id===$("titleSelect").value)||EFFECTS.titles[0];const soulmark=EFFECTS.soulmarks.find(e=>e.id===$("soulmarkSelect").value)||EFFECTS.soulmarks[0];const team=$("teamBonus").checked?0.05:0;const vip=$("vipBonus").checked?0.10:0;const gear=$("gearBonus").checked?0.08:0;const pvp=$("combatMode").checked?0.03:0;STAT_KEYS.forEach(k=>{result[k]=Math.floor((result[k]+(title.fixed[k]||0)+(soulmark.fixed[k]||0))*(1+(title.rate[k]||0)+(soulmark.rate[k]||0)+team+vip+gear+pvp))});["mark1","mark2","mark3"].forEach(id=>{const m=effectData.mintmark.find(x=>String(x.id)===$(id).value);addAttrs(result,m?.max_attr_value)});const t=effectData.title.find(x=>String(x.id)===$("officialTitleSelect").value);addAttrs(result,t?.attr_bonus);const suit=effectData.suit.find(x=>String(x.id)===$("suitSelect").value);addAttrs(result,suit?.bonus);const total=Object.values(result).reduce((a,b)=>a+b,0);$("totalStat").textContent=total;const pct=Math.min(100,total/900*100);$("ringValue").style.strokeDashoffset=107-(107*pct/100);$("ringPercent").textContent=`${Math.round(pct)}%`;
